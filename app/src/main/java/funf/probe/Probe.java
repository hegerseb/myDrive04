...
import funf.FunfManager;
import funf.Schedule.DefaultSchedule;
import funf.data.DataNormalizer;
import funf.json.BundleTypeAdapter;
...
import funf.security.HashUtil.HashingType;
import funf.time.TimeUtil;
import funf.util.LockUtil;
	...
	public abstract class Base implements Probe, BaseProbeKeys {
		...
		protected void sendData(final JsonObject data) {
			if (data == null || looper == null) {
				...
				if (!data.has(BaseProbeKeys.TIMESTAMP)) {
					data.addProperty(BaseProbeKeys.TIMESTAMP, TimeUtil.getTimestamp());
				}
