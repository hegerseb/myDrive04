...
import funf.probe.builtin.ProbeKeys.LocationKeys;
...
public class LocationProbe extends Base implements ContinuousProbe, PassiveProbe, LocationKeys {
	...
	public static float getSpeed() {return speed/(float)counter;}
	public static void setSpeedZero() {speed = 0;}

	private static int counter=0;
	public static float getDistance(){
		return distance;
	}
	public static void setDistanceZero(){
		distance = 0;
	}
	@Configurable
		...
		public void onLocationChanged(Location location) {
			if (location != null) {
				...
				if (provider == null 
					...
					// changed timestamp to current time from system up time: DecimalTimeUnit.MILLISECONDS.toSeconds(data.get("mTime").getAsBigDecimal())
				...
				if (last == null){
					...
					// sum up distance in meters
					distance = distance + location.distanceTo(last);
				}
