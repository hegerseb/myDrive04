...
import funf.config.Configurable;
...
import funf.probe.Probe.RequiredProbes;
import funf.probe.builtin.ProbeKeys.BaseProbeKeys;
import funf.probe.builtin.ProbeKeys.ActivityKeys;
	...
	private class ActivityCounter implements DataListener {
		...
		private void intervalReset() {
			...
			if (varianceSum >= 10.0f) {
				data.addProperty(ActivityKeys.ACTIVITY_LEVEL, ActivityKeys.ACTIVITY_LEVEL_HIGH);
			} else if (varianceSum < 10.0f && varianceSum > 3.0f) {
				data.addProperty(ActivityKeys.ACTIVITY_LEVEL, ActivityKeys.ACTIVITY_LEVEL_LOW);
			} else {
				data.addProperty(ActivityKeys.ACTIVITY_LEVEL, ActivityKeys.ACTIVITY_LEVEL_NONE);
			}
			sendData(data);
			varianceSum = avg = sum = count = 0;
		}
		...
		public void onDataReceived(IJsonObject completeProbeUri, IJsonObject data) {
			double timestamp = data.get(BaseProbeKeys.TIMESTAMP).getAsDouble();
