    ...
    protected void onEnable() {
        ...
        sensorListener = new SensorEventListener() {
            ...
            public void onSensorChanged(SensorEvent event) {
                // Event wird nur behandelt, wenn es kein Grundrauschen ist
                // Funktioniert nur fuer spezielle Verwendung der SensorProbe fuer Acceleration Meter
                double noise = AppSingleton.getNoise();
                if (event.values[0] > noise || event.values[1]> noise || event.values[2]> noise) {

                    JsonObject data = new JsonObject();
                    // changed timestamp to current time not uptime: TimeUtil.uptimeNanosToTimestamp(event.timestamp)
